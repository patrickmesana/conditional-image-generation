...
def train(gen, disc, training_set_cropped, training_set_full, unsafe_train_n):
    batch_size = 100
...
def fit(disc, x, y, batch_size):
    checkpoint = ModelCheckpoint(filepath=tmp_weights_file_name, verbose=1)
    d_loss = disc.fit(x, y, epochs=10, shuffle='batch', batch_size=batch_size, callbacks=[checkpoint])
    return d_loss
...
def main():
    ...
    disc = model()
    disc.compile(loss='categorical_crossentropy', optimizer=Adam(0.001), metrics=['accuracy'])
    disc.summary()
    gen = generator.model()
    ...
    if not os.path.isfile(weights_file_name):
        train(gen, disc, x_train_input, x_train_target, 50000)
    else:
        ...
        # generated_images = create_noise_images(n)
